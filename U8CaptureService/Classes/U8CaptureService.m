...
#import "U8CaptureService.h"
#import "U8VideoWritter.h"
#import <sys/sysctl.h>
#include <mach/mach_time.h>
#import "U8CaptureConstants.h"

@interface U8CaptureService ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate, AVCaptureDepthDataOutputDelegate, U8VideoWritterDelegate> {
...
@property (nonatomic, strong) AVCaptureDepthDataOutput *depthOutput API_AVAILABLE(ios(11.0));

@property (nonatomic, strong) AVCaptureStillImageOutput *imageOutput;

@property (nonatomic, strong) U8VideoWritter *videoWriter;

@end

@implementation U8CaptureService
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _sessionQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_SESSION, DISPATCH_QUEUE_SERIAL);
        _writtingQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_WRITTING, DISPATCH_QUEUE_SERIAL);
        _outputQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_OUTPUT, DISPATCH_QUEUE_SERIAL);
        ...
        _videoDir = [NSTemporaryDirectory() stringByAppendingPathComponent:U8CVIDEODIR];
...
- (void)dealloc {
    NSLog(@"=============U8CaptureService dealloc=============");
...
- (void)switchCamera {
    ...
    NSError *error = nil;
    AVCaptureDevice *videoDevice = [self _inactiveDevice];
    ...
    _devicePosition = _devicePosition == AVCaptureDevicePositionBack ? AVCaptureDevicePositionFront : AVCaptureDevicePositionBack;
}

- (AVCaptureDevice *)_inactiveDevice {
...
- (void)startRecording {
    dispatch_async(_writtingQueue, ^{
        ...
        _recordURL = [NSURL fileURLWithPath:videoFilePath];
        if (_recordURL) {
            _videoWriter = [[U8VideoWritter alloc] initWithURL:_recordURL VideoSettings:_videoSetting audioSetting:_audioSetting];
...
- (BOOL)_setupSession:(NSError **)error {
    ...
    if (![self _setupFaceOutput:error]) {
        return NO;
    }
    
    
    if (_openDepth && [self depthSupported]) {
        if (@available(iOS 11.0, *)) {
            if (![self _setupDepthOutput:error]) {
                return NO;
            }
        }
    }
...
- (BOOL)_setupVideoInputOutput:(NSError **)error {
    ...
    if (_videoInput) {
        if ([_captureSession canAddInput:_videoInput]) {
            ...
            *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_VIDEO code:-2200 userInfo:@{NSLocalizedDescriptionKey:@"add add video input fail"}];
            return NO;
        }
    } else {
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_VIDEO code:-2201 userInfo:@{NSLocalizedDescriptionKey:@"video input is nil"}];
        return NO;
    }
    ...
    if ([_currentDevice lockForConfiguration:error]) {
        ...
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_VIDEO code:-2203 userInfo:@{NSLocalizedDescriptionKey:@"device lock fail(input)"}];
        return NO;
    }
    ...
    dispatch_queue_t videoQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_VIDEO, DISPATCH_QUEUE_SERIAL);
    [_videoOutput setSampleBufferDelegate:self queue:videoQueue];
    
    if ([_captureSession canAddOutput:_videoOutput]) {
        [_captureSession addOutput:_videoOutput];
    } else {
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_VIDEO code:-2204 userInfo:@{NSLocalizedDescriptionKey:@"device add video output fail(output)"}];
        return NO;
    }
...
- (BOOL)_setupImageOutput:(NSError **)error {
    ...
    if ([_captureSession canAddOutput:_imageOutput]) {
        ...
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2205 userInfo:@{NSLocalizedDescriptionKey:@"device add image output fail(output)"}];
        return NO;
    }
}
...
- (BOOL)_setupAudioInputOutput:(NSError **)error {
    ...
    if (_audioInput) {
        if ([_captureSession canAddInput:_audioInput]) {
            ...
            *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2206 userInfo:@{NSLocalizedDescriptionKey:@"device add audio input fail"}];
            return NO;
        }
    } else {
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2207 userInfo:@{NSLocalizedDescriptionKey:@"audio input is nil"}];
        return NO;
    }
    
    self.audioOutput = [[AVCaptureAudioDataOutput alloc] init];
    dispatch_queue_t audioQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_AUDIO, DISPATCH_QUEUE_SERIAL);
    [_audioOutput setSampleBufferDelegate:self queue:audioQueue];
    
    if ([_captureSession canAddOutput:_audioOutput]) {
        [_captureSession addOutput:_audioOutput];
    } else {
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2208 userInfo:@{NSLocalizedDescriptionKey:@"device add audio output fail"}];
        return NO;
    }
...
- (BOOL)_setupFaceOutput:(NSError **)error {
    self.metadataOutput = [[AVCaptureMetadataOutput alloc] init];
    if ([_captureSession canAddOutput:_metadataOutput]) {
        [_captureSession addOutput:_metadataOutput];
    } else {
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2209 userInfo:@{NSLocalizedDescriptionKey:@"device add meta output fail"}];
        return NO;
    }
    NSArray *metaObjectTypes = @[AVMetadataObjectTypeFace];
    _metadataOutput.metadataObjectTypes = metaObjectTypes;
    dispatch_queue_t metaQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_META, DISPATCH_QUEUE_SERIAL);
    [_metadataOutput setMetadataObjectsDelegate:self queue:metaQueue];
    return YES;
}

- (BOOL)_setupDepthOutput:(NSError **)error API_AVAILABLE(ios(11.0)) {
    ...
    if ([_captureSession canAddOutput:_depthOutput]) {
        ...
        *error = [NSError errorWithDomain:U8_CAPTURESERVICE_DOMAIN_IMAGE code:-2215 userInfo:@{NSLocalizedDescriptionKey:@"device add depth output fail"}];
        return NO;
    }
    dispatch_queue_t depthQueue = dispatch_queue_create(U8_CAPTURESERVICE_QUEUE_DEPTH, DISPATCH_QUEUE_SERIAL);
    [_depthOutput setDelegate:self callbackQueue:depthQueue];
    return YES;
}

- (BOOL)_clearVideoFile:(NSError **)error {
    NSString *videoDir = [NSTemporaryDirectory() stringByAppendingPathComponent:U8CVIDEODIR];
    BOOL isDir = NO;
    BOOL exited = [[NSFileManager defaultManager] fileExistsAtPath:videoDir isDirectory:&isDir];
    if (isDir && exited) {
        if (![[NSFileManager defaultManager] removeItemAtPath:videoDir error:error]) {
            return NO;
        }
    }
    return YES;
...
- (void)_captureSessionNotification:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:AVCaptureSessionDidStartRunningNotification]) {
        ...
        NSLog(@"handle other notification : %@",notification.name);
    }
}
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputMetadataObjects:(NSArray<__kindof AVMetadataObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    for (AVMetadataObject *face in metadataObjects) {
        @autoreleasepool {
            AVMetadataFaceObject *faceObject = (AVMetadataFaceObject *)[self.previewLayer transformedMetadataObjectForMetadataObject:face];
            if (faceObject) {
                [transformedFaces addObject:faceObject];
            }
        }
    }
    @autoreleasepool {
...
#pragma mark - U8VideoWritterDelegate
...
- (void)videoWritter:(U8VideoWritter *)writter completeWriting:(NSError *)error {
    if (error) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(captureService:recorderDidFailWithError:)]) {
            [self.delegate captureService:self recorderDidFailWithError:error];
        }
